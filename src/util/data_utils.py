...
def parse_example_proto(example_serialized, has_3d=False):
    ...
    feature_map = {
        ...
        'image/visibility':
        tf.FixedLenFeature((1, 32), dtype=tf.int64),
        'image/x':
        tf.FixedLenFeature((1, 32), dtype=tf.float32),
        'image/y':
        tf.FixedLenFeature((1, 32), dtype=tf.float32),
        'image/face_pts':
        tf.FixedLenFeature(
    ...
    # For reading the new tf records generated by us!!!
    indices = [3, 2, 1, 8, 7, 6, 27, 26, 25, 19, 18, 17, 14, 15]
    features['image/x'] = tf.gather(features['image/x'], indices, axis=1)
    features['image/y'] = tf.gather(features['image/y'], indices, axis=1)
    features['image/visibility'] = tf.gather(features['image/visibility'], indices, axis=1)
